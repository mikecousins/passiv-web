describe('Login using created data from registration', () => {
  it('Log in Success', () => {
    cy.intercept('POST', '/api/v1/auth/login/', { fixture: 'user.json' }).as(
      'Success',
    );

    cy.intercept('GET', '/api/v1', { fixture: 'api_v1.json' }).as('API poke');

    cy.intercept('GET', '/help', { fixture: 'api_v1.json' }).as('API poke');

    cy.intercept('POST', '/api/v1/settings/', (req) => {
      req.reply((res) => {
    ...
    cy.get('@userFixture').then((user) => {
      ...
      cy.contains('Total Holdings').should('be.visible');
